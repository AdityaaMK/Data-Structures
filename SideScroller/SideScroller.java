...
public class SideScroller extends JPanel implements KeyListener, Runnable {
    ...
    public SideScroller() {
        ...
        int[][] locsAndDims = new int[][] {
                ...
                { 4, 64, 31, 53 }, { 34, 64, 31, 51 }, { 62, 64, 31, 51 }, { 92, 64, 31, 51 }, { 127, 64, 37, 51 },
                { 166, 64, 31, 51 }, { 205, 64, 31, 51 }, { 233, 64, 30, 51 }, { 263, 61, 30, 56 }, { 292, 61, 34, 56 },
                { 325, 60, 41, 56 }, { 367, 60, 36, 56 }, };

        int[][] jumpLocsAndDims = new int[][] {
                // x, y, width, height
                { 4, 294, 31, 59 }, // standing
                { 35, 300, 29, 58 }, // jumping
                { 62, 301, 38, 56 }, { 100, 301, 36, 56 }, { 140, 303, 41, 50 }, { 183, 304, 49, 47 },
                { 230, 303, 42, 50 }, // falling
                { 278, 302, 37, 54 }, { 321, 303, 33, 56 }, { 4, 363, 35, 64 }, { 42, 365, 36, 63 },
                { 168, 361, 25, 55 }, };
        ...
        for (int x = 0; x < 12; x++) {
            aladdinJumping[x] = aladdinSheet.getSubimage(jumpLocsAndDims[x][0], jumpLocsAndDims[x][1],
            ...
            if (right) {
                ...
                for (int c = currentColumn; c <= currentColumn + 1; c++) {
                    try {
                        ArrayList<Block> blocks = map.get(c);
                        for (Block b : blocks) {
                            if (hero.getCollisionBox().intersects(b.getCollisionBox()) && hero.sameLevel(b)
                                    && b.isTopBlock()) {
                                hitBlock = true;
                                break;
                            }
